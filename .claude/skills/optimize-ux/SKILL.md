---
name: optimize-ux
description: Analyzes and enhances React components for optimal UX, focusing on smooth interactions using framer-motion, loading states, accessibility, and user feedback (tooltips/toasts).
---

# UX Optimization Expert (Framer Motion Edition)

Your job is to take an existing React component and upgrade its User Experience (UX) without altering its core business logic. You must prioritize `framer-motion` for all significant UI animations and ensure robust user feedback.

When invoked, enforce the following UX pillars:

1. **Framer Motion Animations:** - Import `{ motion, AnimatePresence } from 'framer-motion'`.
...
2. **Perceived Performance & Feedback:**
   ...
   - **Async Operations:** You MUST use `sonner` for toast notifications. Wrap any asynchronous operation (fetches, form submissions, database mutations) in `toast.promise()` to automatically handle and display `loading`, `success`, and `error` states to the user.
...
4. **Strict Routing Rule:**
   - You must enable the View Transitions API by explicitly adding the `viewTransition` prop to all `<Link>` or `<NavLink>` components (e.g., `<Link to="/dashboard" viewTransition>Dashboard</Link>`) and add it as an option to the `Maps()` function from the `useNavigate` hook (e.g., `Maps("/", { viewTransition: true })`).

Output the optimized code and provide a brief bulleted list of the specific Framer Motion properties, shadcn/sonner integrations, and overall UX improvements you implemented.